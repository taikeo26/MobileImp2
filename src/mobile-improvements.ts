...
Hooks.on("drawPrimaryCanvasGroup", () => {
  //@ts-ignore Foundry V14 public canvas API
...
Hooks.once("renderChatLog", (app: any) => {
  if (!app.element) return;

  let touchWhenFocused = false;
  const form = app.element.querySelector(".chat-form") as HTMLFormElement | null;
  const textarea = form?.querySelector("#chat-message") as HTMLTextAreaElement | null;
  const controls = form?.querySelector(".chat-controls");

  if (!form || !textarea || !controls || form.querySelector("#chat-form--send")) {
    return;
  }
  ...
  btn.addEventListener("click", async (evt) => {
    ...
    try {
      await app.processMessage(message);
      textarea.value = "";
      ...
      console.error("Mobile Improvements | Failed to send chat message", error);
    }
  });
